...
def label_function_absolute_8(val):
    if val < -30:
        return 0
    elif val <-20:
        return 1
...
folder_path = rf"ThesisFinalData\larger_scaled_images_rolling_window_6_limit_90\all images"

for i in range(len(df)-rolling_window_size+1):
    start_time = perf_counter()
    rolling_window = df.iloc[i:i+rolling_window_size].reset_index(drop=True)
    max_span_calculated = max(df["high"][i:i + rolling_window_size]) - min(df["low"][i:i + rolling_window_size])

    if max_span_calculated > max_span:
        data_labels.append(math.nan)
        continue
    # print(rolling_window)

    open_to_close_after = rolling_window["open_to_close_after"][rolling_window_size-1]

    data_label = label_function_absolute_8(open_to_close_after)
    ...
    # # will be represented by pink color candlestick
    # col2 = 'pink'
    #
    ...
    #
    #
    # plt.savefig(rf"{folder_path}\image_{i}.png")
    #
    # plt.clf()
    # plt.close()
    #
    ...
    # # plt.show()
    #
    # img = cv2.imread(rf"{folder_path}\image_{i}.png")
    #
    # # print(img.shape)
    #
    #
    ...
    # resized_img = cv2.resize(cropped_img, (120, 400), interpolation=cv2.INTER_AREA)
    # grayscale_img = cv2.cvtColor(resized_img, cv2.COLOR_BGR2GRAY)
    #
    # cv2.imwrite(rf"{folder_path}\image_{i}_cropped.png", cropped_img)
    #
    # # print(f"{round((perf_counter() - start_time) * 1000, 2)} ms")
    #
    # cv2.imwrite(rf"{folder_path}\image_{i}_resized.png", resized_img)
    #
    # # print(f"{round((perf_counter() - start_time) * 1000, 2)} ms")
    #
    # cv2.imwrite(rf"{folder_path}\image_{i}_grayscale.png", grayscale_img)
    #
    # # print(f"{round((perf_counter() - start_time) * 1000, 2)} ms")
...
os.chdir("D:\Pycharm Projects\ThesisFinal\Game Theoretical Adversarial Deep Learning")
df.to_csv("data/data_labels_rolling_window_6_absolute_8_limit_90.csv", index=False)
...
data_labels_df = pd.read_csv(rf"ThesisFinal\Game Theoretical Adversarial Deep Learning\data\data_labels_rolling_window_6_absolute_8_limit_90.csv")

data_labels = list(data_labels_df["data_labels"])

folder_path = rf"ThesisFinalData\larger_scaled_images_rolling_window_6_limit_90"

train_files = os.listdir(rf"{folder_path}\train\images")
...
train_data_labels_df = DataFrame()
train_data_labels_df["data_labels"] = train_data_labels
train_data_labels_df.to_csv(rf"{folder_path}\train\train_data_labels_absolute_8_limit_90.csv", index=False)

test_files = os.listdir(rf"{folder_path}\test\images")
...
test_data_labels_df = DataFrame()
test_data_labels_df["data_labels"] = test_data_labels
test_data_labels_df.to_csv(rf"{folder_path}\test\test_data_labels_absolute_8_limit_90.csv", index=False)
...
# os.chdir(r"D:\Pycharm Projects")
#
# parent_folder = "ThesisFinalData\larger_scaled_images_rolling_window_6_limit_90"
#
# images_list = os.listdir(rf"{parent_folder}\all images")
#
...
#     print(f"1__{index}")
#     index += 1
#     shutil.copy(rf"{parent_folder}\all images\{image}",
#                 rf"{parent_folder}\train\images\{image}")
#
# index = 0
...
#     index += 1
#     shutil.copy(rf"{parent_folder}\all images\{image}",
#                 rf"{parent_folder}\test\images\{image}")
#
...
#
#
# images_list = os.listdir(rf"ThesisFinalData\larger_scaled_images_rolling_window_1_limit_90\all images")
#
# images_list = [image for image in images_list if image.endswith("grayscale.png")]
#
# train_images, test_images = train_test_split(images_list, test_size=0.2, random_state=42)
...
#     print(f"1__{index}")
#     index += 1
#     shutil.copy(rf"ThesisFinalData\larger_scaled_images_rolling_window_1_limit_90\all images\{image}",
#                 rf"ThesisFinalData\larger_scaled_images_rolling_window_1_limit_90\train\images\{image}")
#
# index = 0
# for image in test_images:
#     print(f"2__{index}")
#     index += 1
#     shutil.copy(rf"ThesisFinalData\larger_scaled_images_rolling_window_1_limit_90\all images\{image}",
#                 rf"ThesisFinalData\larger_scaled_images_rolling_window_1_limit_90\test\images\{image}")
...
#     date = df["date"][i]
#     max_span = max(df["high"][i:i+rolling_window_size]) - min(df["low"][i:i+rolling_window_size])
#     max_span_list.append((date, max_span))
#
# print(sorted(max_span_list, reverse=True, key=lambda x: x[1])[:9000])